...
passengers %>%
  mutate(FamSize = Parch + SibSp)  # Create a new column FamSize
...
passengers %>%
  group_by(Sex) %>% 
  summarise(
    meanFare = mean(Fare, na.rm = TRUE),  # Average Fare per Sex
    numSurv = sum(Survived, na.rm = TRUE) # Total number of survivors per Sex
  )


#5 

quantile(diabetes_data$skin, probs= c(0.1, 0.3, 0.5, 0.6), na.rm = TRUE)
```